...
public class CPHInline
{
	...
	private void HandlePresentViewers()
    {
        ...
        if (CPH.TryGetArg("users", out List<Dictionary<string, object>> users) && users != null)
        {
            ...
            {
                if (userDict.TryGetValue("userName", out object userNameObj) && userDict.TryGetValue("id", out object userIdObj))
                {
	...
	public void CheckAndRollOverVofMLeaderboard()
	{
		...
		if (leaderboard.Count > 0)
		{
			...
			if (winners.Count > 0)
			{
				CPH.SetGlobalVar("VofM_CurrentWinner", winners[0]["UserName"], true);
